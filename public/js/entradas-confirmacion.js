...
var contenedoresCodigo = document.getElementsByTagName('*');

for (var indice = 0; indice < contenedoresCodigo.length; indice++) {
    var elementoCodigo = contenedoresCodigo[indice];

    if (!elementoCodigo.hasAttribute('data-codigo')) {
        continue;
    }
    ...
    new QRCode(elementoCodigo, {
        ...
        text:       elementoCodigo.dataset.codigo,
        // parseInt devuelve NaN si el style no tiene width definido; en ese caso usamos 220 px
        width:      parseInt(elementoCodigo.style.width)  || 220,
        height:     parseInt(elementoCodigo.style.height) || 220,
