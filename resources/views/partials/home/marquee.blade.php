@php
$marqueeItems = [
    'Esta noche se rompe',
    'Charlotte de Witte · 09 May',
    'Primavera Sound · phase 3',
    'No te lo pierdas',
    'BCN never sleeps',
    'Lista negra · membership',
    'El mejor techno de la escena',
    'Pacha · Bad Bunny · 21 Jun',
];
@endphp

<div style="overflow:hidden;border-top:1px solid var(--line);border-bottom:1px solid var(--line);padding:18px 0;">
  <div class="marquee-track">
    @foreach(array_merge($marqueeItems, $marqueeItems, $marqueeItems) as $item)
      <span class="display" style="font-size:clamp(36px,5vw,72px);color:var(--ink);opacity:{{ $loop->index % 3 === 1 ? '1' : '0.45' }};display:inline-flex;align-items:center;gap:32px;">
        {{ $item }}
        <span style="color:var(--magenta);font-size:0.6em;">✦</span>
      </span>
    @endforeach
  </div>
</div>
